...
public:
    ...
    [[nodiscard]] const std::size_t rank() const noexcept{
        return shape_.size();
...
int main(){
 ...
 std::cout << "Test 1\n";
  ...
  assert(test_5_threw); 
  Tensor scaler({}, {5});
  assert(scaler.rank()== 0);
  assert(scaler.numel()== 1);
  assert(scaler.shape()== std::vector<std::size_t>{});
  ...
  Tensor overflow({std::numeric_limits<std::size_t>::max(), 2}, {});
    ...
    assert(std::string(error.what()) == "tensor element count overflow size_t");
    test_11_threw = true;
  }
